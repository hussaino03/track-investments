...
# Adjust the import path to access the parser module
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from parser.parser import extract_investment_info
...
class TestParser(unittest.TestCase):
    def test_extraction(self):
        test_cases = [
            ...
            ("User", "Just dropped $300 on Philips shares")
        ]
        ...
        for i, (user_name, user_input) in enumerate(test_cases):
            result = extract_investment_info(user_name, user_input)  # Adjusted function reference
            print(result)
